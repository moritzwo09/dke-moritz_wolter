...
def pdftohtml_test(subpath):
    ...
    output_dir = filePath.replace('.pdf', '')
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)

    abs_input_path = os.path.abspath(filePath)
    abs_output_dir = os.path.abspath(output_dir)

    # Mount the directory that contains the file
    mount_dir = os.path.dirname(abs_input_path)

    # Run docker command to convert PDF to HTML
    docker_command = (
        f'docker run --platform linux/amd64 -ti --rm '
        f'-v {mount_dir}:/pdf '
        f'-w /pdf moritzwolter/my-pdf2htmlex:latest '
        f'pdf2htmlEX --zoom 1.3 {os.path.basename(filePath)}'
    )
